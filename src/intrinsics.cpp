...
static Value *emit_unboxed_coercion(jl_codectx_t &ctx, Type *to, Value *unboxed, Value *dest = NULL, bool volatile_store = false)
{
...
static Value *emit_unbox(jl_codectx_t &ctx, Type *to, const jl_cgval_t &x, jl_value_t *jt, Value *dest, bool volatile_store)
{
    ...
    if (!x.ispointer() || c) { // already unboxed, but sometimes need conversion
        return emit_unboxed_coercion(ctx, to, c ? c : x.V, dest, volatile_store);
    }
...
static jl_cgval_t emit_pointerref(jl_codectx_t &ctx, jl_cgval_t *argv)
{
    ...
    if (ety == (jl_value_t*)jl_any_type) {
        ...
        ctx.builder.CreateMemCpy(strct, thePtr, size, 1);
...
static jl_cgval_t emit_pointerset(jl_codectx_t &ctx, jl_cgval_t *argv)
{
    ...
    if (ety == (jl_value_t*)jl_any_type) {
        ...
        ctx.builder.CreateMemCpy(ctx.builder.CreateGEP(T_int8, thePtr, im1),
                             data_pointer(ctx, x), size, align_nb);
    }
    else {
        bool isboxed;
        Type *ptrty = julia_type_to_llvm(ety, &isboxed);
